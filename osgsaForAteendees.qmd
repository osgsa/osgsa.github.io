...
```{r}
makeMyPlots <- function(myInput1, myInput2) {
  myOutput <- myInput1 * myInput2
  return(myOutput)
}
...
rmarkdown::paged_table(filteredData)
```
## Second, make a simple example *without* a function
```{r}
ggplot(filteredData, aes(x = flipper_length_mm, y = body_mass_g, color = sex)) + 
  geom_point()
```
...
plot <- ggplot(filteredData, aes(x = flipper_length_mm, y = body_mass_g, color = sex)) + 
  geom_point()
...
makeMyPlots <- function(speciesIn, yearIn) {
  #filter for speciesIn, yearIn
  #make plot called "plotoOutput"
  return(plotOutput)
}
...
## My preferred way
```{r}
lakeDataClean <- lakeDataCols %>% 
  mutate(Value = as.numeric(Value)) 
lakesWide <- pivot_wider(lakeDataClean, names_from = Variable, values_from = Value) 
rmarkdown::paged_table(lakesWide)
...
lakeDataClean <- lakeDataCols %>% 
  mutate(Value = as.numeric(Value))
lakesWide <- pivot_wider(lakeDataClean, names_from = Variable, values_from = Value, values_fn = mean) 
rmarkdown::paged_table(lakesWide)
ggplot(lakesWide, aes(x = `Temperature, water`, y = `Dissolved oxygen saturation`)) +
  geom_point()
myModel <- lm(`Dissolved oxygen saturation` ~ `Temperature, water`, data = lakesWide)
summary(myModel)
```
```{r}
lakeDataClean
...
run_smk <- function(site, var, depth) {
 # Filter selected data
  byVar <- lakes %>% 
    ...
    filter(Depth <= depth) %>% 
    filter(!is.na(Month))
    #This just removes years without data
    if (length(unique(byVar$Year)) > 2) {
      #This is the SMK Call
    k <- kendallSeasonalTrendTest(byVar$Value ~ byVar$Month + byVar$Year)
    print(site)
...
k <- run_smk(var = "pH", depth = 1.5, site = "11NPSWRD_WQX-ISRO_07")
k
```
